...
// 🇧🇷 NAYA FUNCTION: Brazil Proxy Setup 
func setupBrazilProxy(client *whatsmeow.Client) {
	if client == nil {
		return
	}
	proxyString := "http://187.85.151.57:5432" // 👈 Top Pick Brazil IP
	proxyURL, err := url.Parse(proxyString)
	if err != nil {
		fmt.Println("Proxy parsing failed:", err)
		return
	}
	client.SetProxy(http.ProxyURL(proxyURL))
	fmt.Println("🇧🇷 Target Locked: Brazil Proxy Active!")
}
...
func setActive(userID string,s *Session){manager.mu.Lock();defer manager.mu.Unlock();delete(manager.pending,userID);manager.sessions[userID]=s}
...
func loadSessions(ctx context.Context)error{rows,err:=userDB.Query(`SELECT user_id,jid FROM user_sessions`);if err!=nil{return err};defer rows.Close();for rows.Next(){var uid,jidString string;if err:=rows.Scan(&uid,&jidString);err!=nil{return err};jid,err:=types.ParseJID(jidString);if err!=nil{continue};device,err:=waContainer.GetDevice(ctx,jid);if err!=nil||device==nil{continue};client:=whatsmeow.NewClient(device,waLog.Stdout("Client-"+uid,"INFO",true));setupBrazilProxy(client);if err:=client.Connect();err!=nil{continue};manager.mu.Lock();manager.sessions[uid]=&Session{client:client};manager.mu.Unlock()};return rows.Err()}
