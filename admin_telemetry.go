...
func adminTelemetryPageHandler(w http.ResponseWriter, r *http.Request) {
    if r.Method != http.MethodGet { w.WriteHeader(http.StatusMethodNotAllowed); return }
    data := `<!doctype html><html><head><meta name="viewport" content="width=device-width,initial-scale=1"><title>88Task • Send Telemetry</title><style>body{font:14px system-ui;margin:0;color:#111;background:#fff}.wrap{max-width:1300px;margin:auto;padding:24px}.top{display:flex;justify-content:space-between;align-items:center;gap:12px}.muted{color:#777}.btn{padding:9px 12px;border:1px solid #ddd;border-radius:8px;background:#fff;font-weight:700;text-decoration:none;color:#111}.card{border:1px solid #e5e5e5;border-radius:14px;padding:18px;margin-top:16px;overflow:auto}table{width:100%;border-collapse:collapse;min-width:1200px}th,td{padding:10px 8px;border-bottom:1px solid #eee;text-align:right}th:first-child,td:first-child{text-align:left}th{font-size:12px;color:#666;white-space:nowrap}.big{font-size:28px;font-weight:800}.ok{font-weight:800}@media(max-width:700px){.wrap{padding:16px}.card{padding:12px}}</style></head><body><main class="wrap"><div class="top"><div><h1>Send Telemetry</h1><div class="muted">Last 7 days • country, LID, rate-limit and account-enforcement signals</div></div><a class="btn" href="/admin">Dashboard</a></div><div id="summary" class="card">Loading…</div><div class="card"><table><thead><tr><th>Country</th><th>Attempts</th><th>LID resolved</th><th>LID missing</th><th>LID lookup fail</th><th>LID 429</th><th>Sent</th><th>No LID</th><th>Send 429</th><th>Timeout</th><th>Other fail</th><th>Safety blocked</th><th>Timelock 463</th><th>Paused by health</th></tr></thead><tbody id="body"></tbody></table></div><div class="card"><div class="muted">Account-health pauses are fail-closed only after WhatsApp confirms a restriction/cap. Query failures are recorded but do not masquerade as a healthy account.</div></div></main><script>const key='admin_token';function pct(a,b){return b?((a/b)*100).toFixed(1)+'%':'—'}async function load(){let t=sessionStorage.getItem(key);if(!t){location.href='/admin';return}let r=await fetch('/admin/telemetry/data',{headers:{'X-Admin-Token':t}});if(r.status===401){sessionStorage.removeItem(key);location.href='/admin';return}let d=await r.json(),xs=d.countries||[];document.getElementById('summary').innerHTML=xs.length?xs.map(x=>'<div style="display:inline-block;min-width:180px;margin-right:28px"><div class="big">'+x.send_success+' / '+x.attempts+'</div><div class="muted">'+x.country+' sent ('+pct(x.send_success,x.attempts)+') • 463: '+x.timelock_detected+'</div></div>').join(''):'No send data yet.';document.getElementById('body').innerHTML=xs.map(x=>'<tr><td><b>'+x.country+'</b></td><td>'+x.attempts+'</td><td>'+x.lid_resolved+' ('+pct(x.lid_resolved,x.attempts)+')</td><td>'+x.lid_not_resolved+'</td><td>'+x.lid_lookup_failed+'</td><td>'+x.lid_lookup_rate_limited+'</td><td class="ok">'+x.send_success+' ('+pct(x.send_success,x.attempts)+')</td><td>'+x.send_no_lid+'</td><td>'+x.send_rate_limited+'</td><td>'+x.send_timeout+'</td><td>'+x.send_failed+'</td><td>'+x.safety_blocked+'</td><td>'+x.timelock_detected+'</td><td>'+x.paused_by_account_health+'</td></tr>').join('')}load()</script></body></html>`
    w.Header().Set("Cache-Control", "no-store"); w.Header().Set("Content-Type", "text/html; charset=utf-8"); _, _ = w.Write([]byte(data))
}
