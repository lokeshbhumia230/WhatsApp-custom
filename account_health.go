...
func adminAccountHealthDataHandler(w http.ResponseWriter,r *http.Request){enableCORS(w);w.Header().Set("Content-Type","application/json");if _,ok:=requireAdmin(w,r);!ok{return};uid:=strings.TrimSpace(r.URL.Query().Get("user_id"));if uid==""{http.Error(w,"user_id is required",400);return};_=json.NewEncoder(w).Encode(getAccountHealth(uid))}
func adminAccountHealthRefreshHandler(w http.ResponseWriter,r *http.Request){enableCORS(w);w.Header().Set("Content-Type","application/json");if _,ok:=requireAdmin(w,r);!ok{return};uid:=strings.TrimSpace(r.URL.Query().Get("user_id"));s:=getSession(uid);if uid==""||s==nil||s.client==nil{http.Error(w,"connected WhatsApp account not found",404);return};_=json.NewEncoder(w).Encode(accountHealthSnapshot(uid,s.client,true))}
