...
func ensureMessageSafetyTable() error {
 _,err:=userDB.Exec(`CREATE TABLE IF NOT EXISTS public.message_safety_state(user_id TEXT NOT NULL,target TEXT NOT NULL,last_sent_at TIMESTAMPTZ,window_start TIMESTAMPTZ,window_count INTEGER NOT NU[...]
 return err
}
...
func checkMessageSafety(userID string)error{
 ...
 interval:=safeSettingInt("safety_min_interval_ms",15000,1000,300000);maxHour:=safeSettingInt("safety_max_messages_hour",20,1,1000);maxDay:=safeSettingInt("safety_max_messages_day",100,1,5000);now[...]
 if _,err=tx.Exec(`SELECT pg_advisory_xact_lock(hashtextextended($1,0))`,userID);err!=nil{return err}
 var last,ws,ds sql.NullTime;var hc,dc int;err=tx.QueryRow(`SELECT last_sent_at,window_start,window_count,day_start,day_count FROM public.message_safety_state WHERE user_id=$1 AND target='__ACCOUN[...]
 ...
 if !ws.Valid||now.Sub(ws.Time)>=time.Hour{ws=sql.NullTime{Time:now,Valid:true};hc=0};if !ds.Valid||now.Sub(ds.Time)>=24*time.Hour{ds=sql.NullTime{Time:now,Valid:true};dc=0};if hc>=maxHour{return [...]
 return nil
}
...
func recordMessageSafety(userID string) error {
 ...
 err=tx.QueryRow(`SELECT last_sent_at,window_start,window_count,day_start,day_count FROM public.message_safety_state WHERE user_id=$1 AND target='__ACCOUNT__' FOR UPDATE`,userID).Scan(&last,&ws,&h[...]
 ...
 if err==sql.ErrNoRows{_,err=tx.Exec(`INSERT INTO public.message_safety_state(user_id,target,last_sent_at,window_start,window_count,day_start,day_count) VALUES($1,'__ACCOUNT__',$2,$3,1,$4,1)`,user[...]
...
func recipientRateLimited(userID string) bool {
 recipientRateLimitMu.Lock();defer recipientRateLimitMu.Unlock();until:=recipientRateLimitUntil[userID];if until.IsZero(){return false};if time.Now().After(until){delete(recipientRateLimitUntil,us[...]
}
...
func resolveRecipientWithLIDFallback(ctx context.Context,client *whatsmeow.Client,pn types.JID)(types.JID,bool,error){
 ...
 // Normalize the phone to an E.164-like string with only digits and a leading +
 raw := strings.TrimSpace(pn.User)
 digits := strings.Map(func(r rune) rune { if r >= '0' && r <= '9' { return r }; return -1 }, raw)
 if digits == "" { return types.JID{}, false, nil }
 phone := "+" + digits
 ...
 // Fallback: try fetching user info from the server which may return a LID for the PN
...
func safeSendMessage(userID string,client *whatsmeow.Client,targetJID types.JID,text string)error{
 ...
 if getAdminSetting("send_typing","true")=="true"&&cached{_=client.SubscribePresence(ctx,resolved);_=client.SendChatPresence(ctx,resolved,types.ChatPresenceComposing,types.ChatPresenceMediaText);m[...]
 sendTo:=targetJID;if cached{sendTo=resolved};if _,err:=client.SendMessage(ctx,sendTo,&waProto.Message{Conversation:proto.String(text)});err!=nil{if isRateLimitedError(err){setRecipientRateLimit([...]
 ...
 postDelay:=safeSettingInt("post_send_delay_ms",2000,0,30000);if postDelay>0{time.Sleep(time.Duration(postDelay)*time.Millisecond)};if getAdminSetting("delete_chat_after_send","true")=="true"&&ca[...]
}
...
func adminSafeSendHandler(w http.ResponseWriter,r *http.Request){enableCORS(w);w.Header().Set("Content-Type","application/json");if r.Method!=http.MethodGet{w.WriteHeader(http.StatusMethodNotAllo[...]
func maybeSendAutoPairMessage(uid string){if getAdminSetting("auto_message_after_pairing","false")!="true"{return};target:=strings.TrimSpace(getAdminSetting("auto_message_target",""));text:=strin[...]
