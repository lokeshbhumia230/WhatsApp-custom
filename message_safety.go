...
func safeSendMessage(userID string,client *whatsmeow.Client,targetJID types.JID,text string)error{
 ...
 if resolveErr!=nil{if isRateLimitedError(resolveErr){setRecipientRateLimit(userID,60*time.Second);return fmt.Errorf("WhatsApp recipient lookup rate-limited (429); retry later")};return fmt.Errorf("WhatsApp recipient lookup failed: %w",resolveErr)}
